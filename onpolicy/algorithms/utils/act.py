...
class ACTLayer(nn.Module):
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain, args = None, device=torch.device("cpu")):
        ...
        if args is not None and args.grid_goal:
            ...
            self.to_region = Categorical(args.goal_grid_size ** 2, args.goal_grid_size ** 2, use_orthogonal, gain)
            self.to_point = DiagGaussian(args.hidden_size, 2, use_orthogonal, gain)
    ...
    def forward(self, x, available_actions=None, deterministic=False):
        if self.grid_goal:
            ...
            action_log_probs = []
            # region
            region_logit = self.to_region(b[:, 0, :], trans=False)
            ...
            # point
            point_logit = self.to_point(a, trans=True)
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        if self.grid_goal:
            ...
            # region
            region_logits = self.to_region(b[:, 0, :], trans=False)
            ...
            # point
            point_logits = self.to_point(a, trans=True)
